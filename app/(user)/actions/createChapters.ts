'use server';
import { generateChaptersByAI } from '@/configs/AIModels';
...
export async function createChaptersContent(course: Course) {
	try {
		...
		chapters?.forEach(async (chapter, index) => {
			...
			Course Name: ${course?.courseOutputByAI?.courseName}, Chapter: ${chapter?.chapterName} 
			with array of object (in JSON Format) with fields as title, explanation on give chapter in detail, 
			Code Example (Code field in <precode> format) if applicable and using ${course?.language}.`;
